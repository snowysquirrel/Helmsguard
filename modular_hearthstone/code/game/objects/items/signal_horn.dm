...
/obj/item/signal_horn/proc/sound_horn(mob/living/user)
	...
	switch(user.job)
		...
		if("Town Sheriff", "Watchman", "Master-At-Arms", "Man at Arms")
	...
	for(var/mob/living/player in GLOB.player_list)
		...
		switch(localarea)
			...
			if("azure basin")
				placetext = " in the The Helmsguard Basin!"
			if("wilderness")
				placetext = " in the The Helmsguard Grove!"
			if("bog", "dense bog")
				placetext = " in the The Terrorbog!"
			if("coast", "coastforest")
				placetext = " on the Helmsguard Coast!"
		...
		switch(user.job)
			...
			if("Marshall", "Watchman", "Master-At-Arms", "Man at Arms")
