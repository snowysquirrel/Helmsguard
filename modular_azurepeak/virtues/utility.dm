...
/datum/virtue/utility/resident
	name = "Resident"
	desc = "I'm a resident of Helmsguard. I have an account in the city's treasury and a home in the city."
	...
	if(recipient.mind?.assigned_role == "Adventurer" || recipient.mind?.assigned_role == "Mercenary" || recipient.mind?.assigned_role == "Court Agent")
		...
		if(spawn_area)
			...
			if(length(possible_chairs))
				...
				to_chat(recipient, span_notice("As a resident of Helmsguard, you find yourself seated at a chair in the local tavern."))
			else
				...
				if(length(possible_spawns))
					...
					to_chat(recipient, span_notice("As a resident of Helmsguard, you find yourself in the local tavern."))
