...
//	src << browse(null, "window=lobby_window")
	...
	for(var/datum/job/job in SSjob.occupations)
		if(istype(job, /datum/job/roguetown/wretch))
			continue
		if(!job)
			continue
		var/readiedas = 0
					...
					if(!(player.client.ckey in GLOB.hiderole))
						if(player.client.prefs.real_name)
							...
							if(istype(job, /datum/job/roguetown/hand))
								if(player != src)
									if(client.prefs.job_preferences["Lord Castellan"] == JP_HIGH)
								...
								for(var/mob/dead/new_player/Lord in GLOB.player_list)
									if(Lord.client.prefs.job_preferences["Lord Castellan"] == JP_HIGH)
