...
/obj/structure/roguemachine/scomm/attack_right(mob/user)
	...
	if(SSticker.rulertype == "")
		contents += "<center>LORD ELECTOR'S DECREES<BR>"
	else
		contents += "<center>LADY ELECTOR'S DECREES<BR>"
...
/obj/structure/roguemachine/scomm/MiddleClick(mob/living/carbon/human/user)
	if(.)
		return
	if((HAS_TRAIT(user, TRAIT_GUARDSMAN) || (user.job == "Warden") || (user.job == "Squire") || (user.job == "Marshal") || (user.job == "Lord Elector") || (user.job == "Knight Captain") || (user.job == "Lady Elector")))
