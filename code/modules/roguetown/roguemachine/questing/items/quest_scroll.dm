...
/obj/item/paper/scroll/quest/proc/update_quest_text()
	...
	switch(assigned_quest.quest_type)
		...
		if(QUEST_KILL, QUEST_MINIBOSS)
			...
			scroll_text += "<b>Last Seen Location:</b> [assigned_quest.target_spawn_area ? "Reported sighting in [assigned_quest.target_spawn_area] region." : "Reported sighting in Helmsguard region."]<br>"
		if(QUEST_CLEAR_OUT)
			scroll_text += "<b>Objective:</b> Eliminate [assigned_quest.target_amount] [initial(assigned_quest.target_mob_type.name)].<br>"
			scroll_text += "<b>Infestation Location:</b> [assigned_quest.target_spawn_area ? "Reported sighting in [assigned_quest.target_spawn_area] region." : "Reported infestations in Helmsguard region."]<br>"
		if(QUEST_COURIER)
